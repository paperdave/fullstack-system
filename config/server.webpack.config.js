...
if (fs.existsSync(path.join(SOURCE_DIR, 'server.webpack.config.js'))) {
  config = deepmerge(config, require(path.join(SOURCE_DIR, 'server.webpack.config.js')), {
    clone: false,
  });
}
if (fs.existsSync(path.join(SOURCE_DIR, 'webpack.server.config.js'))) {
  config = deepmerge(config, require(path.join(SOURCE_DIR, 'webpack.server.config.js')), {
    clone: false,
  });
}
